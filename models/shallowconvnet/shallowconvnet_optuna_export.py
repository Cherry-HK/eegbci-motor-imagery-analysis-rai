...
MODEL_DIR = os.path.join("models", "shallowconvnet")
OPTUNA_DIR = os.path.join(MODEL_DIR, "shallowconvnet_optuna")
BEST_CONFIG_CSV = os.path.join(OPTUNA_DIR, "all_trials_summary.csv")
MODEL_PATH = os.path.join(MODEL_DIR, "shallowconvnet_optuna_model.pth")
METADATA_PATH = os.path.join(MODEL_DIR, "shallowconvnet_optuna_metadata.json")
...
def load_best_configuration(path):
    best_row = load_best_row(path, "models/shallowconvnet/shallowconvnet_optuna.py")
    return {
        "trial_number": int(best_row["trial_number"]),
        "temporal_filters": parse_numeric(best_row["temporal_filters"]),
        "filter_length": parse_numeric(best_row["filter_length"]),
        "pool_length": parse_numeric(best_row["pool_length"]),
        "pool_stride": parse_numeric(best_row["pool_stride"]),
...
if __name__ == "__main__":
    ...
    print("Loaded best ShallowConvNet Optuna configuration:", best_config)
    print("Device:", DEVICE)
    ...
    metadata = {
        "model_type": "shallowconvnet_optuna",
    ...
    print("Saved ShallowConvNet Optuna weights to:", MODEL_PATH)
