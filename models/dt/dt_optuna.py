...
RESULTS_DIR = os.path.join("models", "dt", "dt_optuna")
...
def plot_top_trials(path, rows, top_n=10):
    ...
    plt.title(f"Top {len(top_rows)} DT Optuna Trials")
...
def write_best_summary(path, best_result, total_trials, top_rows):
    ...
    for row in top_rows[:3]:
        lines.append(
            f"rank {row['rank']}: criterion={row['criterion']}, max_depth={row['max_depth']}, "
            ...
            f"mean_accuracy={row['mean_accuracy']:.4f}, mean_f1={row['mean_f1']:.4f}, mean_auc={row['mean_auc']:.4f}"
        )
...
if __name__ == "__main__":
    ...
    def objective(trial):
        config = suggest_dt_params(trial)
        result = run_loso_dt(X, y, subjects, progress_label=f"trial {trial.number + 1}/{N_TRIALS}", **config)
    ...
    write_csv(os.path.join(RESULTS_DIR, "all_trials_summary.csv"), ranked_rows, ["rank", "trial_number", "criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "all_trials_fold_results.csv"), fold_rows, ["trial_number", "criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    write_csv(os.path.join(RESULTS_DIR, "top_10_trials.csv"), ranked_rows[:10], ["rank", "trial_number", "criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "best_configuration_per_subject.csv"), sorted(best_fold_rows, key=lambda row: row["subject"]), ["trial_number", "criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    save_confusion_matrix_csv(os.path.join(RESULTS_DIR, "best_confusion_matrix.csv"), best_result["overall_confusion_matrix"])
    save_confusion_matrix_plot(os.path.join(RESULTS_DIR, "best_confusion_matrix.png"), best_result["overall_confusion_matrix"], "Best DT Optuna Configuration: Overall LOSO Confusion Matrix")
    plot_top_trials(os.path.join(RESULTS_DIR, "top_10_trials.png"), ranked_rows, top_n=10)
    plot_subject_accuracy(os.path.join(RESULTS_DIR, "best_per_subject_accuracy.png"), best_result["per_subject_rows"], "Best DT Optuna Configuration: LOSO Accuracy per Subject")
    plot_subject_metric_comparison(os.path.join(RESULTS_DIR, "best_per_subject_metrics.png"), best_result["per_subject_rows"], "Best DT Optuna Configuration: Per-Subject LOSO Metrics")
