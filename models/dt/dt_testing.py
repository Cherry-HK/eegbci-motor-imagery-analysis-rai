...
RESULTS_DIR = os.path.join("models", "dt", "dt_testing")
...
def run_loso_dt(X, y, subjects, *, criterion="gini", max_depth=5, min_samples_split=10, min_samples_leaf=5, class_weight="balanced", csp_components=6, progress_label=""):
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(X, y, groups=subjects), 1):
        ...
        model = DecisionTreeClassifier(criterion=criterion, max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, class_weight=class_weight, random_state=42)
...
def plot_top_combinations(path, rows, top_n=10):
    top_rows = rows[:top_n]
    labels = [f"#{row['rank']} {row['criterion']}" for row in top_rows]
    scores = [row["mean_accuracy"] for row in top_rows]
    plt.figure(figsize=(11, 6))
    bars = plt.bar(labels, scores, color="#2563eb")
    plt.ylabel("Mean Accuracy")
    plt.title(f"Top {len(top_rows)} DT Hyperparameter Combinations")
...
SEARCH_CONFIG = {"criterion": ["gini", "entropy", "log_loss"], "max_depth": [5, 10, None], "min_samples_split": [2, 10], "min_samples_leaf": [1, 5], "class_weight": [None, "balanced"], "csp_components": [6]}
...
if __name__ == "__main__":
    ...
    for index, combo in enumerate(combinations, 1):
        result = run_loso_dt(X, y, subjects, progress_label=f"combo {index}/{len(combinations)}", **combo)
        summary_rows.append({"criterion": result["criterion"], "max_depth": result["max_depth"], "min_samples_split": result["min_samples_split"], "min_samples_leaf": result["min_samples_leaf"], "class_weight": result["class_weight"], "csp_components": result["csp_components"], "mean_accuracy": result["mean_accuracy"], "std_accuracy": result["std_accuracy"], "mean_f1": result["mean_f1"], "mean_auc": result["mean_auc"]})
        for fold_row in result["fold_rows"]:
            fold_rows.append({"criterion": result["criterion"], "max_depth": result["max_depth"], "min_samples_split": result["min_samples_split"], "min_samples_leaf": result["min_samples_leaf"], "class_weight": result["class_weight"], "csp_components": result["csp_components"], **fold_row})
    ...
    best_fold_rows = [row for row in fold_rows if row["criterion"] == best_row["criterion"] and row["max_depth"] == best_row["max_depth"] and row["min_samples_split"] == best_row["min_samples_split"] and row["min_samples_leaf"] == best_row["min_samples_leaf"] and row["class_weight"] == best_row["class_weight"] and row["csp_components"] == best_row["csp_components"]]
    write_csv(os.path.join(RESULTS_DIR, "all_combinations_summary.csv"), ranked_rows, ["rank", "criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "all_combinations_fold_results.csv"), fold_rows, ["criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    write_csv(os.path.join(RESULTS_DIR, "top_10_combinations.csv"), ranked_rows[:10], ["rank", "criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "best_configuration_per_subject.csv"), sorted(best_fold_rows, key=lambda row: row["subject"]), ["criterion", "max_depth", "min_samples_split", "min_samples_leaf", "class_weight", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    save_confusion_matrix_csv(os.path.join(RESULTS_DIR, "best_confusion_matrix.csv"), best_result["overall_confusion_matrix"])
    save_confusion_matrix_plot(os.path.join(RESULTS_DIR, "best_confusion_matrix.png"), best_result["overall_confusion_matrix"], "Best DT Configuration: Overall LOSO Confusion Matrix")
    plot_top_combinations(os.path.join(RESULTS_DIR, "top_10_combinations.png"), ranked_rows, top_n=10)
    plot_subject_accuracy(os.path.join(RESULTS_DIR, "best_per_subject_accuracy.png"), best_result["per_subject_rows"], "Best DT Configuration: LOSO Accuracy per Subject")
    plot_subject_metric_comparison(os.path.join(RESULTS_DIR, "best_per_subject_metrics.png"), best_result["per_subject_rows"], "Best DT Configuration: Per-Subject LOSO Metrics")
    with open(os.path.join(RESULTS_DIR, "best_configuration_summary.txt"), "w", encoding="utf-8") as handle:
        handle.write(
            "\n".join(
                [
                    "Decision Tree combined-search summary",
                    "=" * 40,
