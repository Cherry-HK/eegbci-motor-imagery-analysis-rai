...
OPTUNA_DIR = os.path.join(MODEL_DIR, "eegnet_optuna")
BEST_CONFIG_CSV = os.path.join(OPTUNA_DIR, "all_trials_summary.csv")
MODEL_PATH = os.path.join(MODEL_DIR, "eegnet_optuna_model.pth")
METADATA_PATH = os.path.join(MODEL_DIR, "eegnet_optuna_metadata.json")
...
def load_best_configuration(path):
    best_row = load_best_row(path, "models/eegnet/eegnet_optuna.py")
    return {
...
if __name__ == "__main__":
    ...
    print("Loaded best EEGNet Optuna configuration:", best_config)
    print("Device:", DEVICE)
    ...
    print("Saved EEGNet Optuna weights to:", MODEL_PATH)
