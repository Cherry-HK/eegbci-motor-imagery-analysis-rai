...
OPTUNA_DIR = os.path.join(MODEL_DIR, "lr_optuna")
BEST_CONFIG_CSV = os.path.join(OPTUNA_DIR, "all_trials_summary.csv")
MODEL_PATH = os.path.join(MODEL_DIR, "lr_optuna_model.joblib")
METADATA_PATH = os.path.join(MODEL_DIR, "lr_optuna_metadata.json")
...
def load_best_configuration(path):
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Optuna summary CSV not found: {path}. Run models/lr/lr_optuna.py first."
        )
...
if __name__ == "__main__":
    ...
    print("Loaded best LR Optuna configuration:", best_config)
