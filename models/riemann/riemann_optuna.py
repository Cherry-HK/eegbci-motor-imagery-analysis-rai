...
RESULTS_DIR = os.path.join("models", "riemann", "riemann_optuna")
...
def plot_top_trials(path, rows, top_n=10):
    ...
    plt.title(f"Top {len(top_rows)} Riemann Optuna Trials")
...
def write_best_summary(path, best_result, total_trials, top_rows, skipped_rows):
    lines = [
        "RIEMANN OPTUNA TUNING SUMMARY",
        "=" * 70,
        f"Total Optuna trials attempted: {total_trials}",
        f"Valid trials completed: {len(top_rows)}",
        f"Skipped trials: {len(skipped_rows)}",
        "",
    ...
    for row in top_rows[:3]:
        lines.append(f"rank {row['rank']}: cov_estimator={row['cov_estimator']}, ts_metric={row['ts_metric']}, C={row['C']}, solver={row['solver']}, class_weight={row['class_weight']}, mean_accuracy={row['mean_accuracy']:.4f}, mean_f1={row['mean_f1']:.4f}, mean_auc={row['mean_auc']:.4f}")
    with open(path, "w", encoding="utf-8") as handle:
        handle.write("\n".join(lines))


def suggest_riemann_params(trial):
    return {
        "cov_estimator": trial.suggest_categorical("cov_estimator", OPTUNA_SEARCH_SPACE["cov_estimator"]),
        "ts_metric": trial.suggest_categorical("ts_metric", OPTUNA_SEARCH_SPACE["ts_metric"]),
        "c_value": trial.suggest_float("c_value", 0.05, 1.0, log=True),
...
if __name__ == "__main__":
    trial_summaries, fold_rows, skipped_rows = [], [], []
    best_result_holder = {"result": None}

    def objective(trial):
        config = suggest_riemann_params(trial)
        try:
            result = run_loso_riemann(X, y, subjects, progress_label=f"trial {trial.number + 1}/{N_TRIALS}", **config)
        except ValueError as error:
            skipped_rows.append({"trial_number": trial.number, **config, "error": str(error)})
            raise optuna.TrialPruned(str(error))
        trial_summaries.append({"rank": 0, "trial_number": trial.number, "cov_estimator": result["cov_estimator"], "ts_metric": result["ts_metric"], "C": result["C"], "solver": result["solver"], "class_weight": result["class_weight"], "mean_accuracy": result["mean_accuracy"], "std_accuracy": result["std_accuracy"], "mean_f1": result["mean_f1"], "mean_auc": result["mean_auc"]})
        for fold_row in result["fold_rows"]:
            fold_rows.append({"trial_number": trial.number, "cov_estimator": result["cov_estimator"], "ts_metric": result["ts_metric"], "C": result["C"], "solver": result["solver"], "class_weight": result["class_weight"], **fold_row})
    ...
    write_csv(os.path.join(RESULTS_DIR, "all_trials_summary.csv"), ranked_rows, ["rank", "trial_number", "cov_estimator", "ts_metric", "C", "solver", "class_weight", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "all_trials_fold_results.csv"), fold_rows, ["trial_number", "cov_estimator", "ts_metric", "C", "solver", "class_weight", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    write_csv(os.path.join(RESULTS_DIR, "top_10_trials.csv"), ranked_rows[:10], ["rank", "trial_number", "cov_estimator", "ts_metric", "C", "solver", "class_weight", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "best_configuration_per_subject.csv"), sorted(best_fold_rows, key=lambda row: row["subject"]), ["trial_number", "cov_estimator", "ts_metric", "C", "solver", "class_weight", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    if skipped_rows:
        write_csv(os.path.join(RESULTS_DIR, "skipped_trials.csv"), skipped_rows, ["trial_number", "cov_estimator", "ts_metric", "c_value", "solver", "class_weight", "error"])
    save_confusion_matrix_csv(os.path.join(RESULTS_DIR, "best_confusion_matrix.csv"), best_result["overall_confusion_matrix"])
    save_confusion_matrix_plot(os.path.join(RESULTS_DIR, "best_confusion_matrix.png"), best_result["overall_confusion_matrix"], "Best Riemann Optuna Configuration: Overall LOSO Confusion Matrix")
    plot_top_trials(os.path.join(RESULTS_DIR, "top_10_trials.png"), ranked_rows, top_n=10)
    plot_subject_accuracy(os.path.join(RESULTS_DIR, "best_per_subject_accuracy.png"), best_result["per_subject_rows"], "Best Riemann Optuna Configuration: LOSO Accuracy per Subject")
    plot_subject_metric_comparison(os.path.join(RESULTS_DIR, "best_per_subject_metrics.png"), best_result["per_subject_rows"], "Best Riemann Optuna Configuration: Per-Subject LOSO Metrics")
    write_best_summary(os.path.join(RESULTS_DIR, "best_configuration_summary.txt"), best_result, N_TRIALS, ranked_rows, skipped_rows)
    with open(os.path.join(RESULTS_DIR, "study_metadata.json"), "w", encoding="utf-8") as jsonfile:
        json.dump({"optimizer": "optuna", "n_trials": N_TRIALS, "best_trial_number": study.best_trial.number if ranked_rows else None, "best_value": study.best_value if ranked_rows else None, "best_params": study.best_trial.params if ranked_rows else None}, jsonfile, indent=2)
