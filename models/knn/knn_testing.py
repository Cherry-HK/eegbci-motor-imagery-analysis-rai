...
RESULTS_DIR = os.path.join("models", "knn", "knn_testing")
...
def run_loso_knn(X, y, subjects, *, n_neighbors=5, weights="distance", metric="minkowski", p=2, csp_components=6, progress_label=""):
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(X, y, groups=subjects), 1):
        ...
        kwargs = {"n_neighbors": n_neighbors, "weights": weights, "metric": metric}
        if metric == "minkowski":
            kwargs["p"] = p
        model = Pipeline([("scaler", StandardScaler()), ("knn", KNeighborsClassifier(**kwargs))])
    ...
    return {
        "n_neighbors": n_neighbors, "weights": weights, "metric": metric, "p": p, "csp_components": csp_components,
...
def plot_top_combinations(path, rows, top_n=10):
    top_rows = rows[:top_n]
    labels = [f"#{row['rank']} {row['metric']}" for row in top_rows]
    scores = [row["mean_accuracy"] for row in top_rows]
    plt.figure(figsize=(11, 6))
    bars = plt.bar(labels, scores, color="#2563eb")
    plt.ylabel("Mean Accuracy")
    plt.title(f"Top {len(top_rows)} KNN Hyperparameter Combinations")
...
SEARCH_CONFIG = {"n_neighbors": [3, 5, 7, 9], "weights": ["uniform", "distance"], "metric": ["euclidean", "manhattan", "minkowski"], "p": [1, 2, 3], "csp_components": [6]}
...
if __name__ == "__main__":
    ...
    for index, combo in enumerate(combinations, 1):
        result = run_loso_knn(X, y, subjects, progress_label=f"combo {index}/{len(combinations)}", **combo)
        summary_rows.append({"n_neighbors": result["n_neighbors"], "weights": result["weights"], "metric": result["metric"], "p": result["p"], "csp_components": result["csp_components"], "mean_accuracy": result["mean_accuracy"], "std_accuracy": result["std_accuracy"], "mean_f1": result["mean_f1"], "mean_auc": result["mean_auc"]})
        for fold_row in result["fold_rows"]:
            fold_rows.append({"n_neighbors": result["n_neighbors"], "weights": result["weights"], "metric": result["metric"], "p": result["p"], "csp_components": result["csp_components"], **fold_row})
    ...
    best_fold_rows = [row for row in fold_rows if row["n_neighbors"] == best_row["n_neighbors"] and row["weights"] == best_row["weights"] and row["metric"] == best_row["metric"] and row["p"] == best_row["p"] and row["csp_components"] == best_row["csp_components"]]
    write_csv(os.path.join(RESULTS_DIR, "all_combinations_summary.csv"), ranked_rows, ["rank", "n_neighbors", "weights", "metric", "p", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "all_combinations_fold_results.csv"), fold_rows, ["n_neighbors", "weights", "metric", "p", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    write_csv(os.path.join(RESULTS_DIR, "top_10_combinations.csv"), ranked_rows[:10], ["rank", "n_neighbors", "weights", "metric", "p", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "best_configuration_per_subject.csv"), sorted(best_fold_rows, key=lambda row: row["subject"]), ["n_neighbors", "weights", "metric", "p", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    save_confusion_matrix_csv(os.path.join(RESULTS_DIR, "best_confusion_matrix.csv"), best_result["overall_confusion_matrix"])
    save_confusion_matrix_plot(os.path.join(RESULTS_DIR, "best_confusion_matrix.png"), best_result["overall_confusion_matrix"], "Best KNN Configuration: Overall LOSO Confusion Matrix")
    plot_top_combinations(os.path.join(RESULTS_DIR, "top_10_combinations.png"), ranked_rows, top_n=10)
    plot_subject_accuracy(os.path.join(RESULTS_DIR, "best_per_subject_accuracy.png"), best_result["per_subject_rows"], "Best KNN Configuration: LOSO Accuracy per Subject")
    plot_subject_metric_comparison(os.path.join(RESULTS_DIR, "best_per_subject_metrics.png"), best_result["per_subject_rows"], "Best KNN Configuration: Per-Subject LOSO Metrics")
    with open(os.path.join(RESULTS_DIR, "best_configuration_summary.txt"), "w", encoding="utf-8") as handle:
        handle.write(
            "\n".join(
                [
                    "KNN combined-search summary",
                    "=" * 40,
                    f"Total combinations tested: {len(combinations)}",
                    "",
                    "Best configuration:",
                    f"- n_neighbors: {best_result['n_neighbors']}",
                    f"- weights: {best_result['weights']}",
                    f"- metric: {best_result['metric']}",
                    f"- p: {best_result['p']}",
