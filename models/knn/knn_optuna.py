...
RESULTS_DIR = os.path.join("models", "knn", "knn_optuna")
...
def plot_top_trials(path, rows, top_n=10):
    ...
    plt.title(f"Top {len(top_rows)} KNN Optuna Trials")
...
def write_best_summary(path, best_result, total_trials, top_rows):
    ...
    for row in top_rows[:3]:
        lines.append(
            f"rank {row['rank']}: n_neighbors={row['n_neighbors']}, weights={row['weights']}, metric={row['metric']}, "
            f"p={row['p']}, csp_components={row['csp_components']}, mean_accuracy={row['mean_accuracy']:.4f}, "
...
def suggest_knn_params(trial):
    metric = trial.suggest_categorical("metric", OPTUNA_SEARCH_SPACE["metric"])
    params = {
        "n_neighbors": trial.suggest_int("n_neighbors", 5, 15, step=2),
        "weights": trial.suggest_categorical("weights", OPTUNA_SEARCH_SPACE["weights"]),
        "metric": metric,
        "p": "unused",
        "csp_components": trial.suggest_categorical("csp_components", OPTUNA_SEARCH_SPACE["csp_components"]),
    }
    if metric == "minkowski":
        params["p"] = trial.suggest_categorical("p_minkowski", [1, 2, 3])
    return params
...
if __name__ == "__main__":
    ...
    def objective(trial):
        config = suggest_knn_params(trial)
        result = run_loso_knn(X, y, subjects, progress_label=f"trial {trial.number + 1}/{N_TRIALS}", **config)
    ...
    write_csv(os.path.join(RESULTS_DIR, "all_trials_summary.csv"), ranked_rows, ["rank", "trial_number", "n_neighbors", "weights", "metric", "p", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "all_trials_fold_results.csv"), fold_rows, ["trial_number", "n_neighbors", "weights", "metric", "p", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    write_csv(os.path.join(RESULTS_DIR, "top_10_trials.csv"), ranked_rows[:10], ["rank", "trial_number", "n_neighbors", "weights", "metric", "p", "csp_components", "mean_accuracy", "std_accuracy", "mean_f1", "mean_auc"])
    write_csv(os.path.join(RESULTS_DIR, "best_configuration_per_subject.csv"), sorted(best_fold_rows, key=lambda row: row["subject"]), ["trial_number", "n_neighbors", "weights", "metric", "p", "csp_components", "fold", "subject", "accuracy", "f1_score", "roc_auc", "support"])
    save_confusion_matrix_csv(os.path.join(RESULTS_DIR, "best_confusion_matrix.csv"), best_result["overall_confusion_matrix"])
    save_confusion_matrix_plot(os.path.join(RESULTS_DIR, "best_confusion_matrix.png"), best_result["overall_confusion_matrix"], "Best KNN Optuna Configuration: Overall LOSO Confusion Matrix")
    plot_top_trials(os.path.join(RESULTS_DIR, "top_10_trials.png"), ranked_rows, top_n=10)
    plot_subject_accuracy(os.path.join(RESULTS_DIR, "best_per_subject_accuracy.png"), best_result["per_subject_rows"], "Best KNN Optuna Configuration: LOSO Accuracy per Subject")
    plot_subject_metric_comparison(os.path.join(RESULTS_DIR, "best_per_subject_metrics.png"), best_result["per_subject_rows"], "Best KNN Optuna Configuration: Per-Subject LOSO Metrics")
